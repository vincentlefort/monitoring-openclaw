...
for fname in ["ntv.json", "ntv_dashboard_payload.json", "kraken.json", "stocks.json", "bot_status.json", "trades.json"]:
...
# 3. HTML read-only scan
print("\n3. HTML Read-Only Enforcement")
if not os.path.exists(INDEX_PATH):
    ...
    sections = ["Kraken Auto Spot NTV", "Breakout 20", "Legacy Bots", "Alerts & Safety",
                "Data Freshness", "Dry-Run Orders", "OpenClaw Monitoring"]
    for sec in sections:
        if sec in html: ok(f"Section '{sec}' present")
        else: error(f"Section '{sec}' MISSING")

# 4. No execution endpoints
print("\n4. Execution Endpoint Check")
with open(INDEX_PATH) as f:
    html = f.read()
...
for word in banned:
    ...
    if matches:
        # Check if it's only in safety alert text
        safe_context = True
        for m in matches:
